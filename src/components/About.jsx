...
const About = () => {
  ...
  return (
    <>
      <div
        className={`bg-secondary bg-opacity-80 rounded-2xl ${styles.paddingX} min-h-[240px] pt-5 mt-10 w-full`}
      >
        <p className={`${styles.sectionSubText}`}>Get to know the Artist</p>
        <h1 className={`${styles.sectionHeadText}`}>About Myself</h1>
      </div>

      <section
        id="about"
        className={`${styles.paddingX} -mt-16 mx-7 max-w-7xl bg-tertiary rounded-[30px] flex max-sm:flex-col justify-end items-center`}
      >
        <div className="flex flex-col max-sm:p-3 justify-start items-center w-1/2 max-sm:justify-between ">
          <p className={`${styles.heroSubText} mt-2 text-white-100`}>
            I am a{" "}
            <span className="text-[#FEA82F]">Professional Photographer.</span> I
            will shoot <br className="sm:block hidden" /> your{" "}
            <span className="text-[#FEA82F]">Portfolio.</span> I am based in
            Islamabad.
          </p>
        </div>
